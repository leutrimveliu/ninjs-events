...
const EventSingle = ({ user_id, filterChanged }) => {
  let { id } = useParams();
  ...
  const getEventsList = async () => {
    if (currentUser) {
      ...
      setTicketLoading(response.message);
  ...
  if (currentUser) {
    if (currentUser.role.includes("user")) {
      if (eventDetails) {
        ...
        if (ticketLoading === "") {
          buttons = <></>;
        } else if (ticketLoading === 1) {
          buttons = <p>You already have booked this event!</p>;
        } else if (ticketLoading === 2) {
          buttons = (
            <>
              ...
              <Link
                to={`/createpayment/${eventDetails._id}`}
                style={{ textDecoration: "none" }}>
                <button className="btn btn-lg w-100 d-block mx-auto btn-success">
                  Ticket
                </button>
              </Link>
            </>
